...
import {
  AuthExpiredError,
  fetchModelList,
  clearCacheByMerchant,
  clearAllPaymentMethodsCache,
} from "@/lib/api";
import { useAuth } from "@/lib/auth-context";

// Port of the "🗑️ Сбросить кеш по мерчантам" / "🗑️ ПОЛНЫЙ сброс кеша" admin
// actions on `MerchantPaymentMethodAdmin` (`app.services.cache_clear_actions_service`).
// Own page (not the generic list/detail views) — needs multi-select, which
// the generic DataTable doesn't support.

type Row = Record<string, unknown>;

export default function MerchantPaymentMethodBulkActionsPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    fetchModelList("merchant-payment-methods", { page: 1, page_size: 100, search: search || undefined })
      .then((data) => {
  ...
  async function handleClearByMerchant() {
    setSaving(true);
    setError(null);
    setResult(null);
    try {
      const { merchants } = await clearCacheByMerchant(Array.from(selectedIds));
      const names = merchants.map((m) => m.name).join(", ");
      setResult(`Кеш методов сброшен для мерчантов: ${names} (обработано ${selectedIds.size} записей).`);
    } catch (err) {
  ...
  return (
    <div className="max-w-2xl space-y-4">
      <div>
        <Link href="/admin/merchant-payment-methods" className="text-sm text-accent hover:underline">
          ← Платёжные методы мерчантов
        </Link>
      ...
      <div className="card space-y-3 p-4">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-semibold">Сбросить кеш по мерчантам выбранных записей</h2>
          ...
          {rows.map((r) => {
            ...
            return (
              <label key={id} className="flex cursor-pointer items-center gap-2 px-3 py-1.5 text-sm hover:bg-black/5 dark:hover:bg-white/5">
                ...
                <span>merchant_id={String(r.merchant_id ?? "")}, payment_method_id={String(r.payment_method_id ?? "")}</span>
              </label>
            );
          })}
        </div>
        <button
          onClick={handleClearByMerchant}
          disabled={saving || selectedIds.size === 0}
          className="rounded-md bg-accent px-4 py-2 text-sm font-medium text-white hover:bg-accent-dark disabled:opacity-60"
        >
          {saving ? "Сброс…" : `Сбросить кеш по мерчантам (${selectedIds.size})`}
        </button>
      </div>
